...
from models.pitt import CLIPTransformer
...
def evaluate(test_loader, transformer, loss_fn):
    ...
    with torch.no_grad():
        transformer.eval()
        test_loss = 0
        for bn, (x0, y, grid, tokens, t, sentence) in enumerate(test_loader):
        #for bn, (x, y, grid, tokens, x0) in enumerate(test_loader):
            ...
            y_pred = transformer(grid.to(device=device), sentence.to(device=device), x0.to(device=device), t.to(device=device))
...
def get_transformer(model_name, neural_operator, config):
    if(config['embedding'] == 'standard'):
        ...
        transformer = CLIPTransformer(500, config['hidden'], config['layers'], config['heads'],
                                    config['num_x'], dropout=config['dropout'], neural_operator=neural_operator,
                                    temporal_neural_operator=temporal_neural_operator).to(device=device)
    return transformer


def get_data(f, config):
    if(config['flnm'] == 'all'):
        train_data = TransformerMultiOperatorDataset(base_path=config['base_path'],
                                split="train",
        ...
        val_data = TransformerMultiOperatorDataset(base_path=config['base_path'],
                                split="val",
                                initial_step=config['initial_step'],
                                reduced_resolution=config['reduced_resolution'],
                                reduced_resolution_t=config['reduced_resolution_t'],
        ...
        test_data = TransformerMultiOperatorDataset(base_path=config['base_path'],
                                split="test",
                                initial_step=config['initial_step'],
                                reduced_resolution=config['reduced_resolution'],
                                reduced_resolution_t=config['reduced_resolution_t'],
...
def run_pretraining(config, prefix):
    ...
    print("\nWEIGHT DECAY: {}\n".format(config['pretraining_weight_decay']))
    ...
    for epoch in tqdm(range(config['pretraining_epochs'])):
        ...
        transformer.train()
        for bn, (x0, y, grid, tokens, t, sentence) in enumerate(train_loader):
        ...
        with torch.no_grad():
            ...
            val_loss = 0
            all_val_preds = []
            for bn, (x0, y, grid, tokens, t, sentence) in enumerate(val_loader):
                # Forward pass: compute predictions by passing the input sequence
                # through the transformer.
                y_pred = transformer(grid.to(device=device), sentence.to(device=device), x0.to(device=device), t.to(device=device),
                                     clip=True)
                ...
                # Compute the loss.
                labels = torch.arange(y_pred.shape[1]).to(device).repeat(y_pred.shape[0], 1)
                val_loss += clip_loss_fn(y_pred, labels).item()
...
if __name__ == '__main__':
    ...
    # Load config
    with open("./configs/clip_pitt_config.yaml", 'r') as stream:
    ...
    os.makedirs("{}{}_{}_{}".format(train_args['results_dir'], train_args['transformer'], train_args['neural_operator'], prefix),
                exist_ok=True)
    shutil.copy("./configs/clip_pitt_config.yaml",
                "{}{}_{}_{}/clip_pitt_config.yaml".format(train_args['results_dir'], train_args['transformer'],
