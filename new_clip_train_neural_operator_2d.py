...
from models.fno import CLIPFNO2d, FNO2d, LLMFNO2d
from models.deeponet import DeepONet2D
from models.vit import VisionTransformer
from models.clip_pretrainer import LLMPretraining, CLIPPretraining, OFormerLLMPretraining, OFormerCLIPPretraining
...
def get_model(model_name, config):
    if(model_name == "fno"):
        model = FNO2d(config['num_channels'], config['modes1'], config['modes2'], config['width'],
                      config['initial_step']+6 if(config['coeff']) else config['initial_step']+1,
                      config['dropout'])
    if(config['embedding'] == 'llm'):
        conditioner = LLMPretraining(llm=config['llm'], embed_dim=config['embed_dim'], im_size=config['im_size'],
                               initial_step=config['initial_step']+5 if(config['coeff']) else config['initial_step'])
    if(config['embedding'] == 'oformerllm'):
        conditioner = OFormerLLMPretraining(llm=config['llm'], embed_dim=config['embed_dim'], im_size=config['im_size'],
                               initial_step=config['initial_step']+5 if(config['coeff']) else config['initial_step'])
    elif(config['embedding'] == 'clip'):
        conditioner = CLIPPretraining(llm=config['llm'], embed_dim=config['embed_dim'], im_size=config['im_size'],
                                initial_step=config['initial_step']+5 if(config['coeff']) else config['initial_step'])
    elif(config['embedding'] == 'oformerclip'):
        conditioner = OFormerCLIPPretraining(llm=config['llm'], embed_dim=config['embed_dim'], im_size=config['im_size'],
                                initial_step=config['initial_step']+5 if(config['coeff']) else config['initial_step'])
...
def new_get_data(f, config, pretraining=False):
    ...
    #    train_data.choose_subset(config['data_name'])
...
def evaluate(test_loader, model, conditioner, loss_fn, navier_stokes=True, config=None):
    ...
    with torch.no_grad():
        ...
        for bn, (xx, grid, sentence_embeddings) in enumerate(test_loader):
            ...
            if(isinstance(model, (FNO2d, OFormer2D))):
                ...
                sentence_embeddings = conditioner(xx, grid, sentence_embeddings).permute(0, 2, 3, 1)
                inp = torch.cat((xx, sentence_embeddings), dim=-1)
            ...
            elif(isinstance(model, LLMFNO2d)):
                if(config['coeff']):
                    nu = coeffs['nu'].unsqueeze(-1)
                    ax = coeffs['ax'].unsqueeze(-1)
                    ay = coeffs['ay'].unsqueeze(-1)
...
def run_pretraining(model_name, config, prefix, ds=None):
    ...
    #f = h5py.File("{}/{}".format(config['base_path'], config['data_name']), 'r')
    f = None
    model_path = path + "/" + model_name + "_pretrain_{}".format(config['seed'])

    # Create the transformer model.
    transformer, conditioner = get_model(model_name, config)
    if(config['pretraining_num_samples'] == 0):
        print("\nNO PRETRAINING\n")
        return transformer, conditioner

    #TODO: Check if model has been pretrained and train from that instead
    if(config['load_pretrained']): # Loading a specific dataset
        if(ds is not None):
            prefix = prefix.replace(ds, "electric_2d")
        #load_path = "./pretrained_clip_modules/{}/{}_{}".format(config['pretraining_num_samples'], model_name, prefix)
        load_path = "./weighted_pretrained_clip_modules/{}/{}_{}".format(config['pretraining_num_samples'], model_name, prefix)
        #load_path = "./noise_weighted_pretrained_clip_modules/{}/{}_{}".format(config['pretraining_num_samples'], model_name, prefix)
        load_path += "/{}_{}_{}_{}.pt".format(config['seed'], config['llm'], config['pretraining_num_samples'], config['embedding'])
        try:
            conditioner.load_state_dict(torch.load(load_path)['model_state_dict'])
            print("\nSUCCESSFULLY LOADED PRETRAINED MODEL")
            return transformer, conditioner
        except FileNotFoundError:
            print("NO PRETRAINED MODEL. RUNNING PRETRAINING.")
        raise
    ...
    for epoch in tqdm(range(config['pretraining_epochs'])):
        ...
        for bn, (x0, grid, coeffs, sentence_embeddings) in enumerate(train_loader):
            ...
            # Current implementation needs us to loop every time...
            #loss = 0
            #for i in range(len(sentence_embeddings)):
            #    #y_pred = transformer(torch.cat((x0, grid), dim=-1).permute(0,3,1,2), sentence_embeddings, True)
            #    y_pred = transformer(inp[i].unsqueeze(0), grid[i].unsqueeze(0), sentence_embeddings[i], True)
...
def run_training(model, conditioner, config, prefix):
    ...
    if(config['embedding'] == 'llm'):
        total_params = sum(p.numel() for p in model.parameters() if p.requires_grad) + \
                       sum(p.numel() for p in conditioner.parameters() if p.requires_grad)
    elif(config['embedding'] == 'oformerllm'):
        if(config['detach']):
            total_params = sum(p.numel() for p in model.parameters() if p.requires_grad) + \
                           sum(p.numel() for p in conditioner.proj_up.parameters() if p.requires_grad)
        else:
            total_params = sum(p.numel() for p in model.parameters() if p.requires_grad) + \
                           sum(p.numel() for p in conditioner.parameters() if p.requires_grad)
    elif(config['embedding'] == 'clip'):
    ...
    if(config['return_text']):
        ...
        _data, _, __ = next(iter(val_loader))
    dimensions = len(_data.shape)
    print('Spatial Dimension', dimensions - 3)
        
    
    if(config['embedding'] == 'llm'):
        optimizer = torch.optim.Adam(list(list(model.parameters()) + list(conditioner.parameters())),
    ...
    for ep in tqdm(range(start_epoch, config['epochs'])):
        ...
        for bn, (xx, grid, sentence_embeddings) in enumerate(train_loader):
            ...
            if(isinstance(model, CLIPFNO2d)):
                if(config['coeff']):
                    ...
                    inp = xx
                print("SENTENCE_EMBEDDINGS: {}".format(sentence_embeddings.requires_grad))
                raise
                im, loss = model.get_loss(inp, yy[...,0], grid, sentence_embeddings, loss_fn)
            elif(isinstance(model, LLMFNO2d)):
                if(config['coeff']):
        ...
        if ep % config['validate'] == 0:
            ...
            with torch.no_grad():
                ...
                for bn, (xx, grid, sentence_embeddings) in enumerate(val_loader):
    ...
    test_vals = []
    test_value = evaluate(test_loader, model, conditioner, loss_fn, navier_stokes, config=config)
    test_vals.append(test_value)
    print("TEST VALUE FROM LAST EPOCH: {0:5f}".format(test_value))
...
if __name__ == "__main__":
    ...
    # Done with 100 X 10... want to run more interesting things now...
    # Running 500,1000 X 10000 to hopefully get more stable results
            ...
            for embedding in ['oformerllm']:
                ...
                #for llm in ['all-mpnet-base-v2']:
                    ...
                    for coeff in [True]:
                        ...
                        #for ds in ['heat', 'burger', 'adv', 'heat,adv,burger']:
                        #for ds in ['burger', 'heat,adv,burger']:
                        for ds in ['heat,adv,burger']:
                        #for ds in ['burger']:
                            print("\n{}\t{}\t{}\t{}\t{}\n".format(ns, pns, embedding, llm, coeff))
                            #if(embedding == 'oformerllm' and llm == 'all-MiniLM-L6-v2' and ds == 'heat'):
                                #print("\nSKIPPING\n")
                                #continue
                            ...
                            for seed in [0]:
                                ...
                                model, conditioner = run_pretraining(model_name, train_args, prefix, ds)
                                run_training(model, conditioner, train_args, prefix)
                            print("Done.")
